...
void RK4_1D(float* x, float* v, float* dx, float* dv, float t, float dt,
	...
	for(size_t i=0U; i<N; ++i){
		dx[i] = dt * one_sixth * (k1_dx[i] + 2.0f * k2_dx[i] + 2.0f * k3_dx[i] + k4_dx[i]);
		dv[i] = dt * one_sixth * (k1_dv[i] + 2.0f * k2_dv[i] + 2.0f * k3_dv[i] + k4_dv[i]);
	}
...
void next_2D(Vector2D* coord, Vector2D* vel, Vector2D* new_coord, Vector2D* new_vel, float dt, size_t N,
	...
	/* Calculating new coordinates */
	size_t size = N * sizeof(Vector2D);
	Vector2D* d_coord = malloc(size);
	Vector2D* d_vel = malloc(size);

	RK4_2D(coord, vel, d_coord, d_vel, 0.0f, dt, dfdx, N);

	for(size_t i=0U; i<N; ++i){
		new_coord[i].x = coord[i].x + d_coord[i].x;
		new_coord[i].y = coord[i].y + d_coord[i].y;

		new_vel[i].x = vel[i].x + d_vel[i].x;
		new_vel[i].y = vel[i].y + d_vel[i].y;
	}
...
void RK4_3D(Vector3D* x, Vector3D* v, Vector3D* dx, Vector3D* dv, float t, float dt,
	...
	for(size_t i=0U; i<N; ++i){
		dx[i].x = dt * one_sixth * (k1_dx[i].x + 2.0f * k2_dx[i].x + 2.0f * k3_dx[i].x + k4_dx[i].x);
		dx[i].y = dt * one_sixth * (k1_dx[i].y + 2.0f * k2_dx[i].y + 2.0f * k3_dx[i].y + k4_dx[i].y);
		dx[i].z = dt * one_sixth * (k1_dx[i].z + 2.0f * k2_dx[i].z + 2.0f * k3_dx[i].z + k4_dx[i].z);
		dv[i].x = dt * one_sixth * (k1_dv[i].x + 2.0f * k2_dv[i].x + 2.0f * k3_dv[i].x + k4_dv[i].x);
		dv[i].y = dt * one_sixth * (k1_dv[i].y + 2.0f * k2_dv[i].y + 2.0f * k3_dv[i].y + k4_dv[i].y);
		dv[i].z = dt * one_sixth * (k1_dv[i].z + 2.0f * k2_dv[i].z + 2.0f * k3_dv[i].z + k4_dv[i].z);
	}
...
void next_3D(Vector3D* coord, Vector3D* vel, Vector3D* new_coord, Vector3D* new_vel, float dt, size_t N,
	     void(*dfdx)(Vector3D*,Vector3D*,Vector3D*,Vector3D*,float,size_t)){
	/* Calculating new coordinates */
	size_t size = N * sizeof(Vector3D);
	Vector3D* d_coord = malloc(size);
	Vector3D* d_vel = malloc(size);

	RK4_3D(coord, vel, d_coord, d_vel, 0.0f, dt, dfdx, N);

	for(size_t i=0U; i<N; ++i){
		new_coord[i].x = coord[i].x + d_coord[i].x;
		new_coord[i].y = coord[i].y + d_coord[i].y;
		new_coord[i].z = coord[i].z + d_coord[i].z;

		new_vel[i].x = vel[i].x + d_vel[i].x;
		new_vel[i].y = vel[i].y + d_vel[i].y;
		new_vel[i].z = vel[i].z + d_vel[i].z;
	}
