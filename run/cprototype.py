...
import numpy as np
from ctypes import c_float, c_int, c_size_t, c_void_p, Structure, POINTER, byref, cdll, CFUNCTYPE
...
class EOMSolver:
    ...
    def _prototype_1D(self):
        ...
        self.next_step.argtypes = [self.c_vec_ptr, self.c_vec_ptr,
                                   self.c_vec_ptr, self.c_vec_ptr, c_float, c_size_t]

    def _prototype_2D(self):
        """
        Prototype the 2D next step function from the C library.
        Assuming function
        `void next_2D(Vector2D* coord, Vector2D* vel, Vector2D* new_coord, Vector2D* new_vel, float dt, size_t N, void (*f)(...));`
        exists in the C library.
        """
        self.next_step = self.lib.next_2D
        self.c_arr = Vector2D*self.NUMBER_OF_PARTICLES # Alias for pointer to an array of Vector2D
        
        CALLBACK_TYPE = CFUNCTYPE(None, POINTER(Vector2D), POINTER(Vector2D), POINTER(Vector2D), POINTER(Vector2D), c_float, c_size_t)
        
        self.next_step.argtypes = [self.c_vec_ptr, self.c_vec_ptr,
                                   self.c_vec_ptr, self.c_vec_ptr, c_float, c_size_t, c_void_p]

    def _prototype_3D(self):
        """
        Prototype the 3D next step function from the C library.
        Assuming function
        `void next_3D(Vector3D* coord, Vector3D* vel, Vector3D* new_coord, Vector3D* new_vel, float dt, size_t N, void (*f)(...));`
        exists in the C library.
        """
        self.next_step = self.lib.next_3D
        self.c_arr= Vector3D*self.NUMBER_OF_PARTICLES # Alias for pointer to an array of Vector3D
        
        CALLBACK_TYPE = CFUNCTYPE(None, POINTER(Vector3D), POINTER(Vector3D), POINTER(Vector3D), POINTER(Vector3D), c_float, c_size_t)
